...
def _pick_background(slug: str, bg_dir: Path) -> Path:
    """Nişe özel bg_<slug>.mp4 varsa onu, yoksa background_1080p.mp4, yoksa ilk MP4'ü seç."""
    ...
    if not backgrounds:
        raise FileNotFoundError(f"backgrounds bulunamadı: {bg_dir}")
    return backgrounds[0]


def run(audio_path: Path, slug: str = "", bg_dir: Path | None = None) -> Path:
    """Background MP4 + audio → YouTube videosu (1920x1080)."""
    ...
    cmd = [
        ...
        "-filter_complex",
        (
            "[0:v]scale=1920:-2,crop=1920:1080,setsar=1[bg];"
