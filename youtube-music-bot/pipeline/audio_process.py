...
def concat_with_crossfade(tracks: list[Path], crossfade_sec: int = 3) -> Path:
    """Concatenate tracks with crossfade, produce single MP3."""
    ...
    if len(normalized) == 1:
        ...
        print(f"[audio] Single track copied: {FINAL_AUDIO}")
        return FINAL_AUDIO
    ...
    print(f"[audio] Merged: {FINAL_AUDIO}")
    return FINAL_AUDIO


def run(track_paths: list[Path]) -> Path:
    print(f"[audio] Processing {len(track_paths)} tracks...")
