# -*- coding: utf-8 -*-
"""
zapier_skill.py — Zapier webhook ile social media paylaşımı.
.env gereken:
  ZAPIER_WEBHOOK_INSTAGRAM=https://hooks.zapier.com/hooks/catch/xxx/yyy/
...
logger = get_logger("skills.zapier")
...
def post_via_zapier(platform: str, video_url: str, caption: str,
    ...
    # Instagram: direkt Graph API (geen Zapier nodig)
    if platform == "ig":
        try:
            from src.skills.instagram_skill import post_reel
            return post_reel(video_url, caption, brand=brand)
        except ImportError:
            pass  # fallback naar webhook hieronder
    """
    platform: 'ig' | 'tt' | 'yt'
    video_url: publiek toegankelijke URL van de video
...
def post_to_all(video_url: str, caption: str, brand: str = "", topic: str = "") -> dict:
    """Post naar alle geconfigureerde platforms. Geeft dict {platform: (ok, msg)}"""
    results = {}
